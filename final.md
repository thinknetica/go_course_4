...
### Возможности сервиса
Сервис должен предоставлять следующие взможности пользователю с помощью HTTP REST API:
1. Принимать на вход длинную ссылку, формировать на её основе короткую на основе собственного домена, сохранять в БД пару ссылок и возвращать пользователю короткую ссылку.
...
Пример:  
Допустим, что наш сервис работает на домене lynks.org. Тогда, если пользователь выполняет запрос:
```
...
3. Микросервис кэширования. Этот микросервис должен ускорить поиск гиперссылок за счёт хранения недавно созданных пар ссылок с БД в памяти. Микросервис кэширования должен предоставлять метод API для получения пары гиперссылок и сохранять полученую пару в БД. Также должен быть метод API для получения длинной ссылки по короткой.
